...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(
        ...
        if isinstance(use_torch_compile, str):
            use_torch_compile = use_torch_compile.strip().lower() in {"1", "true", "yes", "on"}
        self.forward_dtype = PrecisionType.to_dtype(self.config.get('forward_dtype', 'bf16'))
        print(f'Actor forward_dtype={self.forward_dtype}')

        self.compute_entropy_from_logits = (
            torch.compile(verl_F.entropy_from_logits, dynamic=True)
            if use_torch_compile  # use torch compile by default
            else verl_F.entropy_from_logits)

    def _forward_micro_batch(self, micro_batch, temperature, return_entropy: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns: 
        ...
        with torch.autocast(device_type='cuda', dtype=self.forward_dtype):
            ...
            if self.use_remove_padding:
                ...
                entropy = verl_F.entropy_from_logits(logits) if return_entropy else None  # (bsz, response_length)
