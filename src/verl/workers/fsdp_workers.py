...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        use_lora = lora_adapter_path is not None and str(lora_adapter_path) not in ('', 'None', 'null')
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if use_lora:
                ...
                peft_save_and_load._maybe_shard_state_dict_for_tp = (
                    lambda model, state_dict, adapter_name: None
                )
                if self.rank == 0:
                    print(f"Loading PEFT LoRA adapter from {lora_adapter_path}; trainable={role == 'actor'}")
                actor_module = PeftModel.from_pretrained(
                    actor_module,
                    lora_adapter_path,
                    is_trainable=(role == 'actor'),
                )
                if role != 'actor':
                    actor_module.eval()
                if self.rank == 0:
        ...
        if role == 'actor' and optim_config is not None:
            ...
            if self.rank == 0:
                trainable_numel = sum(p.numel() for p in optim_params)
                total_numel = sum(p.numel() for p in actor_module_fsdp.parameters())
                print(f"Actor optimizer trainable params: {trainable_numel:,} / {total_numel:,}")
            actor_optimizer = optim.AdamW(optim_params,
                                          lr=optim_config.lr,
